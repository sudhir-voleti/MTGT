...
with gr.Blocks(title="Panel Traction: Dynamic Account Scoring", theme=gr.themes.Soft()) as app:
    ...
    **Upload panel data, compute monthly traction, cluster trajectories, and visualize dynamics.**
    ...
    numeric_cols_state = gr.State(None)  # Store numeric column list for reuse
    ...
    with gr.Tab("📈 Traction Over Time"):
        ...
        gr.Markdown("*Mean traction quotient per month for each cluster (with segment sizes)*")
    ...
    def generate_trajectory_plot(wide_df, view, account, k):
        """Generate trajectory plot. Returns image path or None."""
        ...
        # Safe period extraction - handle both numeric and string period names
        periods = []
        for c in period_cols:
            try:
                periods.append(int(c.split("_")[1]))
            except (ValueError, IndexError):
                periods.append(c)  # fallback to string
        ...
        non_period_cols = [c for c in wide_df.columns if not c.startswith("Period_") and c != "Cluster"]
        if non_period_cols and account and account != "(None)" and account in wide_df[non_period_cols[0]].values:
            acc_data = wide_df[wide_df[non_period_cols[0]] == account]
            if len(acc_data) > 0:
    ...
    def handle_configure(df, account_col, period_col):
        if df is None or not account_col or not period_col:
            return None, None, None, None, None, gr.CheckboxGroup(choices=["(Configure panel first)"], value=[], interactive=False), gr.CheckboxGroup(choices=["(Configure panel first)"], value=[], interactive=False), gr.CheckboxGroup(choices=["(Configure panel first)"], value=[], interactive=False), "❌ Select both fields."

        if account_col == period_col:
            return None, None, None, None, None, gr.CheckboxGroup(choices=["(Configure panel first)"], value=[], interactive=False), gr.CheckboxGroup(choices=["(Configure panel first)"], value=[], interactive=False), gr.CheckboxGroup(choices=["(Configure panel first)"], value=[], interactive=False), "❌ Account and Period fields must be different."
        ...
        if len(numeric_cols) < 2:
            return None, None, None, None, None, gr.CheckboxGroup(choices=["(Configure panel first)"], value=[], interactive=False), gr.CheckboxGroup(choices=["(Configure panel first)"], value=[], interactive=False), gr.CheckboxGroup(choices=["(Configure panel first)"], value=[], interactive=False), "❌ Need at least 2 numeric variables for traction mapping."

        return (
            df,
            account_col,
            period_col,
            numeric_cols,  # Store for reuse in clear_weights
    ...
    def update_weight_editor(processed_cols, value_vars, access_vars, evidence_vars):
        # processed_cols is a DataFrame state, check if it's None or empty
    ...
    def handle_clear_weights(numeric_cols_list):
        # numeric_cols_list is the stored list of numeric column names
        if numeric_cols_list is None or len(numeric_cols_list) == 0:
            return (
                "",
                gr.CheckboxGroup(choices=["(Configure panel first)"], value=[], label="Value Variables", interactive=False),
                gr.CheckboxGroup(choices=["(Configure panel first)"], value=[], label="Access Variables", interactive=False),
                gr.CheckboxGroup(choices=["(Configure panel first)"], value=[], label="Evidence Variables", interactive=False)
            )
        return (
            "",
            gr.CheckboxGroup(choices=numeric_cols_list, value=[], label="Value Variables", interactive=True),
            gr.CheckboxGroup(choices=numeric_cols_list, value=[], label="Access Variables", interactive=True),
            gr.CheckboxGroup(choices=numeric_cols_list, value=[], label="Evidence Variables", interactive=True)
        )
    ...
    def handle_compute(df, account_col, period_col, weight_text, k):
        ...
        # Cluster centroids (in original traction scale, not standardized)
        # Compute centroids from actual traction values, not standardized
        centroids_data = []
        for i in range(k):
        ...
        # Summary with cluster sizes appended
        summary_display = summary[[account_col, "Cluster_Label", "Mean_Traction", "Std_Traction", "Trend"] + period_cols[:3] + period_cols[-3:]]

        # Account dropdown (limit to 100 for performance)
        account_list = ["(None)"] + sorted(df[account_col].unique().tolist())[:100]

        # Generate initial trajectory plot (Cluster Means view)
        initial_plot = generate_trajectory_plot(wide, "Cluster Means", "(None)", k)

        return (
            f"✅ Computed traction for {len(df):,} rows. Clustered {len(wide):,} accounts into {k} trajectory clusters.",
            scree_path,
            centroids,
            initial_plot,  # FIX: Generate initial plot instead of None!
            summary_display,
            scored_long_df,
            wide,
    ...
    def handle_trajectory_plot(wide_df, view, account, k):
        """Wrapper that calls the shared plot generator."""
        return generate_trajectory_plot(wide_df, view, account, k)
    ...
    def handle_export_report(wide_df, long_df, reflection, centroids, k):
        ...
        # Cluster sizes from wide_df
        cluster_sizes = wide_df["Cluster"].value_counts().sort_index()
        ...
        for i in range(k):
            ...
            report.append(f"  Cluster {i+1}: {size} accounts ({size/len(wide_df)*100:.1f}%)")
    ...
    configure_btn.click(
        ...
        outputs=[df_processed, account_id_col, period_col, numeric_cols_state, value_check, access_check, evidence_check, config_status]
    )
    ...
    view_dropdown.change(
        ...
        inputs=[scored_wide, view_dropdown, account_dropdown_view, k_slider],
