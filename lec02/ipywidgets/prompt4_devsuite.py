# ═══════════════════════════════════════════════════════════════════════
# Generic Customer Segmentation & Traction Audit (ipywidgets)
...
tab5 = widgets.VBox([
    ...
    widgets.HTML("""
    <p><b>Step 1:</b> Check variables for each component. <b>Step 2:</b> Set weights (0–1+). 
    Weight = 0 excludes the variable. Weight > 0 includes it proportionally.</p>
    """),
    widgets.HTML("<p><b>Step 1: Select Variables per Component</b></p>"),
    widgets.HBox([value_check, access_check, evidence_check]),
    widgets.HTML("<p><b>Step 2: Set Weights for Selected Variables</b></p>"),
...
# ═══════════════════════════════════════════════════════════════════════
# TAB 6: TRACTION CALCULATION + REFLECTION + EXPORT
# ═══════════════════════════════════════════════════════════════════════
...
reflection_box = widgets.Textarea(
    value='',
    placeholder='Explain your reasoning for mapping variables to Value/Access/Evidence...',
...
export_status = widgets.HTML(value="<p>Calculate traction and write reflection, then export.</p>")

tab6 = widgets.VBox([
    widgets.HTML("<h2>📊 Traction Calculation</h2>"),
    widgets.HTML("<h3>Traction Results</h3>"),
    widgets.HTML("<p><b>Overall Sample</b> (K=1, no clustering) vs <b>Segment-wise</b> (your named segments)</p>"),
    ...
    ranking_txt,
    widgets.HTML("<h4>📝 Reflection: Why This Mapping?</h4>"),
    widgets.HTML("<p><i>Explain your reasoning for mapping variables to Value/Access/Evidence. What trade-offs did you consider?</i></p>"),
    reflection_box,
    widgets.HTML("<h4>📥 Export Report</h4>"),
...
def handle_run(b):
    ...
    try:
        ...
        with cents_output:
            clear_output()
            display(HTML("<h4>Centroids (Standardized Scale, 3 decimals) — Transposed</h4>"))
...
def handle_apply_names(b):
    ...
    # Update displays with new names
...
def handle_traction(b):
    ...
    def compute_scores(data_norm):
        v_num = sum(data_norm[v].mean() * weights[v]["Value"] for v in value_vars)
        v_den = sum(weights[v]["Value"] for v in value_vars)
        v_score = v_num / v_den if v_den > 0 else 0

        a_num = sum(data_norm[v].mean() * weights[v]["Access"] for v in access_vars)
        a_den = sum(weights[v]["Access"] for v in access_vars)
        a_score = a_num / a_den if a_den > 0 else 0

        e_num = sum(data_norm[v].mean() * weights[v]["Evidence"] for v in evidence_vars)
    ...
    overall_result = pd.DataFrame({
        ...
        "Overall_Sample": [
            len(overall_data), 100.0, round(v_ov, 3), round(a_ov, 3), round(e_ov, 3), round(traction_ov, 4)
        ]
    })
    ...
    if labels is not None:
        ...
        segment_results = []
        for seg in segments:
            seg_data = analysis_norm[analysis_norm["Segment"] == seg]
            v_s, a_s, e_s = compute_scores(seg_data)
            traction_s = v_s * a_s * e_s
            size_count = len(seg_data)
            size_pct = size_count / len(analysis_df) * 100
        ...
        with segment_output:
            clear_output()
            display(HTML("<p>No segmentation run yet.</p>"))
        ranking_txt.value = "No segmentation run yet."

    traction_status.value = f"<p style='color:green;'>✅ Traction computed. Overall={round(traction_ov, 4)}</p>"
    export_btn.disabled = False
    export_status.value = "<p>Click Download to export the full report.</p>"
...
display(widgets.HTML("<p><b>Upload any dataset, segment via K-Means, map to Traction Equation, and export results.</b></p>"))
display(tabs)
