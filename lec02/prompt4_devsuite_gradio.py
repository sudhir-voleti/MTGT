...
# Works with any dataset (DevSuite, FinScale, etc.)
...
with gr.Blocks(title="Caselet: Customer Segmentation & Traction Audit", theme=gr.themes.Soft()) as app:
    ...
    **Upload any dataset, segment via K-Means, map to Traction Equation, and export results.**
    """)
    ...
    with gr.Tab("📊 Traction Calculation"):
        ...
        **Overall Sample** (K=1, no clustering) vs **Segment-wise** (your named segments)
        ...
        export_btn = gr.Button("📄 Download Full Report (Markdown)", variant="secondary", size="lg")
    ...
    # ── Traction calculation (Overall + Segment-wise) ───────────────
    def handle_traction(df_proc, labels, weight_text, segment_names):
        if df_proc is None:
            return "❌ Run segmentation in Tab 3 first.", None, None, ""

        weights = parse_weights(weight_text)
        if not weights:
            return "❌ Set weights in Tab 5 first. Check variables and assign weights.", None, None, ""
        ...
        if not value_vars or not access_vars or not evidence_vars:
            return "❌ Each component needs at least one variable with weight > 0.", None, None, ""
        ...
        return (
            f"✅ Traction computed. Overall={round(traction_ov, 4)}",
            overall_result,
            transposed,
            ranking_text
        )
    ...
    # ── Export report ─────────────────────────────────────────────────
    def handle_export(df_proc, labels, segment_names, weight_text, reflection, traction_status, overall_tbl, segment_tbl, ranking_txt):
        if df_proc is None:
    ...
    calc_btn.click(
        fn=handle_traction,
        inputs=[df_processed, segment_labels_state, weight_editor, segment_names_state],
        outputs=[traction_status, overall_tbl, segment_tbl, ranking_txt]
    )
